...
//Pega o tipo de Glosa para colocar no título.
$query = "SELECT descRes FROM glosa_tipo_glosa WHERE id ='$tipoGlosa'";
$sql = mysqli_query ($conexao, $query);
$row = mysqli_fetch_assoc ($sql);
$tipoGlosaString = utf8_decode($row['descRes']);
...
$reportTitle = "<h3>".$tipoGlosaString." - ".$mesCompetencia."</h3>";
...
$query = "SELECT nomeUsuario, dataInicio, dataFim, duracao, qtd_ocorrencias, numTicket, procedente
FROM glosa_glosa, glosa_usuario WHERE glosa_glosa.usuario = glosa_usuario.id 
AND competencia = $comp AND tipoGlosa = '$tipoGlosa' ORDER BY dataInicio";
...
if (mysqli_affected_rows($conexao) > 0){
		...
		echo "<table>
		<tr>
		<th>Usuário</th><th>Data Inicial</th><th>Data Final</th><th>Duração</th><th>Qtd. Ocorrências</th><th>Ticket</th><th>Procede (S/N)</th>
		</tr>";
		...
		echo "<tr>
		<td>".$row['nomeUsuario']."</td><td>".$dataInicio."</td><td>".$dataFim."</td><td>".$row['duracao']."</td>
...
elseif (mysqli_affected_rows($conexao) == 0){
	echo "<p><h4>Não há Glosas deste tipo cadastradas na competência atual.</h4></p>";
}	
//Fim do relatório.
echo $pageFooter;	
?>