...
// Verifica se houve algum erro com o upload. Se sim, exibe a mensagem do erro
    if ($_FILES['anexoRacap']['error'] != 0) {
        $messageAnexo = "Não foi possível fazer o upload, erro:" . $_UP['erros'][$_FILES['anexoRacap']['error']];
...
// Faz a verificação da extensão do arquivo
    ...
    if (array_search($extensao, $_UP['extensoes']) === false) {
        $messageAnexo = "Por favor, envie arquivos com as seguintes extensões: jpg, png, gif, pdf, doc, zip, rar e 7z";
        return $messageAnexo;
    }
// Faz a verificação do tamanho do arquivo
    if ($_UP['tamanho'] < $_FILES['anexoRacap']['size']) {
        $messageAnexo = "O arquivo enviado é muito grande, envie arquivos de até 15Mb.";
        return $messageAnexo;
    }
...
// Depois verifica se é possível mover o arquivo para a pasta escolhida
    if (move_uploaded_file($_FILES['anexoRacap']['tmp_name'], $_UP['pasta'] . $nome_final)) {
        ...
        $messageAnexo = "Upload efetuado com sucesso!";
    } else {
        // Não foi possível fazer o upload, provavelmente a pasta está incorreta
        $messageAnexo = "Não foi possível enviar o arquivo, tente novamente";
    }
